...
public:
    graph(int num_nodes__) :
        _adj_list(num_nodes__ + STARTS_FROM), num_nodes(num_nodes__)
    , end_node(num_nodes) {}

    void add_edge(int src_node, int dest_node, int weight) {
        _adj_list[src_node].emplace_back(dest_node, weight);
        _adj_list[dest_node].emplace_back(src_node, weight);
    }
...
int graph::dijkstra() {
    ...
    while (!pq.empty()) {
        ...
        for (const auto &[dest, weight] : _adj_list[cur_node]) {
            if (!visited[dest])
